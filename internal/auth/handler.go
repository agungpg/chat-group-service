...
func (h *Handler) Login(c *fiber.Ctx) error {
	...
	fmt.Println("login is running: ", payload)
...
func (h *Handler) CheckIsUsernameOrEmailTaken(c *fiber.Ctx) error {
	var payload CheckUsernameAndEmailDTO
	if err := c.BodyParser(&payload); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"error": "Invalid input",
		})
	...
	availability, err := h.service.CheckIsUsernameOrEmailTaken(c.Context(), payload)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"data":    availability,
			"message": err.Error(),
